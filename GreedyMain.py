...
def GreedyDeploy(reqs):
    reqs = sorted(reqs,key=lambda x:len(x),reverse=True)
    random.seed(100)
    ...
    for req in reqs:
        ...
        for ms in req:
            # node = random.choice(EDGE_NODE_LIST) # different greedy scheme
